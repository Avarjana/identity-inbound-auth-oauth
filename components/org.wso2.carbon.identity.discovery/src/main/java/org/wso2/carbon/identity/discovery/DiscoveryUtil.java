    ...
    public static boolean isUseEntityIdAsIssuerInOidcDiscovery() {
        ...
        if (StringUtils.isEmpty(useEntityIdAsIssuerInDiscovery)) {
            if (log.isDebugEnabled()) {
                log.debug("UseEntityIdAsIssuerInOidcDiscovery property is not configured. Using default value: true");
            }
            return true;
        }
        boolean useEntityId = Boolean.parseBoolean(useEntityIdAsIssuerInDiscovery);
        if (log.isDebugEnabled()) {
            log.debug("UseEntityIdAsIssuerInOidcDiscovery property is set to: {}", useEntityId);
        }
        return useEntityId;
    }
